...
public class PokemonScreenLAB9 extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		java.util.Random ramdom = new Random( System.currentTimeMillis());
		int x = ramdom.nextInt(W);
		int y = ramdom.nextInt(H);
		currentPosx = x - x%STEP_SIZE; // this should be a random position
		currentPosy = y - y%STEP_SIZE; // this should be a random position
		...
		pikachuImage = new Image(new File("icons/25.png").toURI().toString());
	...
	private void movePikachu(double currentPosx2, double currentPosy2) {
		pikachu.relocate(currentPosx2, currentPosy2);		
	}
